...
class DecodedObjectLookup
{
     ...
     */
    template<typename EnumTag, typename = std::enable_if_t<std::is_enum_v<EnumTag>>>
    DecodedObjectLookup operator[](EnumTag tag) const
    {
        return (*this)[static_cast<std::uint32_t>(tag)];
    }
...
class Decoder
{
     ...
     */
    bool loadDictionariesFromDirectory(const std::string &path, std::string error)
    {
